...
namespace OmniConverter
{
    ...
    */
    ...
    public class FLP : MIDI
    {
        ...
        public FLP(MidiFile? loadedFile, IEnumerable<MIDIEvent> metaEvent, string name, long id, string path, TimeSpan timeLength, int tracks, long noteCount, ulong fileSize) : base(loadedFile, metaEvent, name, id, path, timeLength, tracks, noteCount, fileSize)
        { }
        ...
        public static MIDI? Load(long id, string filePath, string name, ParallelOptions parallelOptions, Action<int, int> progressCallback)
        {
            try
            {
                ...
                var midiMetaEvents = GetMetaEvents(midTracks, parallelOptions, ref maxTicks, ref noteCount, progressCallback);
                ...
                return new FLP(midi, mergedMetaEvents, name, id, filePath, TimeSpan.FromSeconds(seconds), midi.TrackCount, noteCount, (ulong)(new FileInfo(filePath).Length));
            }
